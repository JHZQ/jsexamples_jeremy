...
selectDom.addEventListener("change", () => {
    ...
    for (let index = 0; index < collection.length; index++) {
        ...
        // only if it's not found, then add it to the list to display
        if (!found) {
...
btnClearSelectList.addEventListener("click", () => {
    ...
    // clears the text of all selected items
    document.getElementById("listSelected").innerText = "";

    // clears the array arrSelected so it can be selected again
